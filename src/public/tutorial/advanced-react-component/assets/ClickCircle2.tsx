...
function ClickCircle({ parameters, setAnswer }: StimulusParams<any>) {
  ...
  const clickCallback = useCallback((e: React.MouseEvent) => {
    ...
    const isWithinCircle = Math.sqrt((pointer[0] - circelPos[0]) ** 2 + (pointer[1] - circelPos[1]) ** 2) < 10 ? 'Yes' : 'No';

    setAnswer({
      status: true,
      answers: {
        [taskid]: isWithinCircle,
      },
    });
  }, [setAnswer, taskid]);
  ...
  return (
    <div className="Chart__wrapper" ref={ref} onClick={clickCallback} style={{ height: '650px' }}>
      <svg id="click_svg" width={400} height={400}>
        <circle id="Circle" cx={cx} cy={cy} r={r} stroke="black" strokeWidth="2" fill="white" />
      </svg>
    </div>
